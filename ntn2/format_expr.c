...
** Last update Sun Dec  7 22:18:15 2008 christophe sundas
*/
...
char		**str_to_tab_string(char *base, char *expr, char *ops)
{
  char		**tabexpr;
  int		cpt;
  int		nb;

  if ((tabexpr = malloc(sizeof(*tabexpr) * (nb_case(base, expr, ops) + 1) )) == 0)
    exit(-1);
  cpt = 0;
  while (*expr != '\0')
    {
      nb = nb_char_to_add(base, expr, ops, cpt);
      if ((tabexpr[cpt] = malloc (sizeof(**tabexpr) * (nb + 1))) == 0)
	exit(-1);
      my_strlcat(tabexpr[cpt], expr, nb + 1);
      if (need_to_clean(base,ops, tabexpr[cpt]) == 1)
	my_strcpy(tabexpr[cpt], clean_it(base, ops, tabexpr[cpt]));
      cpt = cpt + 1;
      expr = expr + nb ;
    }
  tabexpr[cpt] = NULL;
